...
def create_invitation():
    resp = http_post_json("connections/create-invitation?alias=Issuer", body={"my_label": "Issuer University"}).json()
...
def complete_connection_to_student():
    # Get connections
    conns = http_get("connections?alias=Issuer").json()['results']
    for conn in conns:
        ...
        if conn['state'] == "request" and 'alice' in conn['their_label']:
            ...
            print(resp)
            time.sleep(5)
            latest_conns = http_get("connections?alias=Issuer").json()['results']
            print(latest_conns)
...
def issue_credential_to_student():
    connections = http_get("connections?alias=Issuer").json()['results']
...
@app.route('/reset')
def reset():
    resp = http_get("connections?alias=Issuer").json()
